...
class Library_Management_System_Admin {
    ...
    public function owt_library_enqueue_styles() {
        ...
        if (in_array($page, $plugin_valid_pages)) {
            ...
            wp_enqueue_style("owt-lib-datatables-bootstrap4", OWT_LIBRARY_PLUGIN_URL . 'assets/vendor/datatables/dataTables.bootstrap4.min.css', array(), $this->version, 'all');
        }
    }
    ...
    public function owt_library_enqueue_scripts() {
        ...
        if (in_array($page, $plugin_valid_pages)) {
            ...
            wp_enqueue_script("jquery", OWT_LIBRARY_PLUGIN_URL . 'assets/vendor/jquery/jquery.min.js', array(), $this->version, true);
            ...
            wp_enqueue_script("jquery-datatables", OWT_LIBRARY_PLUGIN_URL . 'assets/vendor/datatables/jquery.dataTables.min.js', array(), $this->version, true);
            // wp_enqueue_script("datatables-bootstrap4", OWT_LIBRARY_PLUGIN_URL . 'assets/vendor/datatables/dataTables.bootstrap4.min.js', array(), $this->version, true);
    ...
    public function owt_library_create_book() {
        ...
        $find_book = array();
        $valid_opt_fn = array("edit", "delete");
    ...
    public function owt_library_ajax_handler() {
		...
		$total_count_student = $wpdb->get_results(
                ...
                if (count($total_count_student) > 250) {
                    $this->json(0, "Students limit exceeded, please contact plugin administrator for paid version.");
                }
...
$total_count_staffs = $wpdb->get_results(
                ...
                if (count($total_count_staffs) > 250) {
                    $this->json(0, "Staff limit exceeded, contact plugin administrator for paid version");
                }
                ...
                //$txtquantity = isset($_REQUEST['txtquantity']) ? sanitize_text_field($_REQUEST['txtquantity']) : "";
                $action = isset($_REQUEST['opt_action']) ? trim($_REQUEST['opt_action']) : "add"; // or it will be edit
                ...
                if ($txt_days_count > 30) {
                    $this->json(0, "Issue days allowed upto 30 days");
                }
                ...
                if ($u_type == "student") {
                    ...
                    if ($dd_st_branch_id > 0) {
                        ...
                        if (!empty($dd_student_id) && $dd_student_id == -1) {
                            ...
                            if (count($has_student_already_book) > 0) {
                                $has_issued = 1;
                            }
                    ...
                    if ($dd_staff_id > 0) {
                        ...
                        if (count($has_staff_already_book) > 0) {
                            $has_issued = 1;
                        }
                ...
                $txt_country = isset($_REQUEST['txt_country']) ? sanitize_text_field($_REQUEST['txt_country']) : -1;
